        ...
        if useNuskell.get() is True:
            if hasNuskell is True:
                ...
                if os.path.exists("nuskellCLI.txt"):
                    os.replace("nuskellCLI.txt", relative_to_assets("tests/nuskellCLI.txt"))

                    with open(relative_to_assets("tests/nuskellCLI.txt"), "r") as file:
                        data = file.read()
                        data = data.replace("tempc", "0.")

                    with open(relative_to_assets("tests/nuskellCLI.txt"), "w") as file:
                        file.write(data)

                print("Nuskell Simulation Complete.")

                file_to_open = relative_to_assets("tests/nuskellCLI.txt")
